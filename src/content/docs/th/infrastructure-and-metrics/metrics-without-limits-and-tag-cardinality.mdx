...
cost ของ custom metric มาจาก **tag cardinality ที่ index ไว้** ไม่ใช่ volume ของการ ingest ดิบ **Metrics without Limits** ให้ ingest tag cardinality สูงอย่าง `pod_name` แบบถูก โดยจ่ายแค่ view cardinality ต่ำอย่าง `by service` และเปิด tag เพิ่มทีหลังเพื่อสืบสวนได้โดยไม่ต้องแตะ instrumentation
</Callout>
