...
description: 'Datadog คำนวณ hit count, error count และ latency percentile จากทุก span ที่ไหลเข้ามาได้ยังไง — ไม่ถูกกระทบจาก ingestion sampling หรือ retention filter — ด้วย distribution metric type ที่ชื่อ DDSketch'
sidebar:
  order: 3
---
...
สิ่งที่**ถูกกระทบ**จากทั้งสอง layer คือความสามารถในการเปิด **example trace** ตัวใดตัวหนึ่งโดยเฉพาะ — เช่น request ที่ช้าที่สุดในชั่วโมงที่ผ่านมา หรือ error ตัวหนึ่งของ error type ใดตัวหนึ่ง การหาและเปิด trace ตัวนั้นขึ้นกับว่ารอดจาก ingestion-time sampling *และ* retention filter เก็บ trace ตัวนั้นไว้หรือเปล่า
