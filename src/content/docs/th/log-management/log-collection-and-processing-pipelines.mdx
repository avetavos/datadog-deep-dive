...
**Processing Pipeline** คือ list ของ **processor** ที่เรียงลำดับไว้ รันกับ log ทุกตัวที่มาถึง intake และ match query `filter` ของ pipeline นั้น ขั้นตอนนี้เกิด*หลัง*จาก log ออกจาก host มาถึง Datadog แล้ว และ*ก่อน*ที่จะถูกเขียนลง index ไหน ๆ — หน้าที่ของตัวเองคือ parse ข้อความที่ไม่มีโครงสร้างให้เป็น attribute ที่มีโครงสร้าง แล้ว enrich log เท่านั้น ไม่ใช่การตัดสินว่า log จะถูกเก็บไว้หรือไม่
