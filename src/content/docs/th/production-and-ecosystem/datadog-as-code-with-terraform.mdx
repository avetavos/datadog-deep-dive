...
บทเรียน dashboard จากโมดูล Infrastructure & Metrics คุยเรื่อง named query, formula และ template variable ในมุมของ UI widget model เดียวกันนี้โผล่มาใน Terraform ผ่าน `datadog_dashboard` และตัวเลือกเพิ่มเติมที่ต้องตัดสินใจตอนเขียน code คือ `layout_type`
...
export const quizTerraformTh = [
  ...
  {
    q: 'ใน resource `datadog_dashboard` ต้องมี block `widget_layout` พร้อม x/y/width/height ของแต่ละ widget ชัดเจนตอนไหน',
    options: [
