...
pattern ที่เห็นในทุกตัวนี้ Datadog แยก "query และอ่าน log ได้" ออกจาก "เปลี่ยนได้ว่าอะไรถูก index, exclude หรือ archive" application engineer ที่กำลัง investigate incident ต้อง search log ได้อย่างอิสระ แต่ไม่จำเป็นต้องมีสิทธิ์ไปเปลี่ยน log source production เสียงดัง ๆ จาก excluded ให้กลายเป็น fully indexed แล้วดันบิล log เดือนหน้าขึ้นสามเท่าแบบเงียบ ๆ ทีม platform หรือ SRE ที่ถือ `logs_modify_indexes` กับ `logs_write_exclusion_filters` ตัดสินใจเรื่องนี้ได้อย่างตั้งใจ โดยคำนึงถึง cost tradeoff ในขณะที่คนอื่นยังมีแค่สิทธิ์ read
